...
def pending_risk_acceptances(request):
    """List risk acceptances awaiting approval for the user."""
    if Dojo_Group_Member.objects.filter(group__name="Risk-Approvers", user=request.user).exists():
        ras = Risk_Acceptance.objects.filter(
            approved=False,
            engagement__in=get_authorized_engagements(Permissions.Risk_Acceptance),
